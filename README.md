...
> A lightweight async UDP bridge enabling communication between Windows and WSL.
...
This project implements a **UDP port bridge** using Python and `asyncio`, allowing Windows applications to communicate with UDP services running inside WSL.
...
6. Idle sessions are automatically cleaned up after a configurable timeout.
...
```bash
python udp_win_wsl_port_bridge.py --listen-port 5060 --wsl-host 172.25.224.1 --wsl-port 5060 --timeout 60
```
...
- The bridge will:
  ...
  - Stop listening on the Windows UDP port
  - Cleanly shutdown the asyncio event loop

This ensures no sockets are left open and the bridge exits gracefully.
...
UDP is connectionless, but many protocols behave in a request-response pattern. Maintaining per-client sockets prevents:
